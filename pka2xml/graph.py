...
class Devices:
  ...
  def __init__(self, nodes):
    # support two possible XML layouts: root may be PACKETTRACER5 or the NETWORK node
    devs = nodes.findall('PACKETTRACER5/NETWORK/DEVICES/DEVICE')
    if not len(devs):
      devs = nodes.findall('NETWORK/DEVICES/DEVICE')
    self.devices = [Device(d) for d in devs]
